...
This file provides guidance to Claude Code when working in this repository or in any repository that references the OIML SMART style guide.

## Read the style guide first

Before drafting, editing, or reviewing any written content for the OIML SMART program, read `WRITING_STYLE.md` in full. The guide defines register, prohibited patterns, content rules, vocabulary, and formatting. The guide is the authoritative source on writing conventions for this project.

## Apply the guide strictly

The guide is not advisory. Its prohibited-pattern and content-rule sections are binding. In particular, the following rules are enforced on every draft.
...
No dollar amounts, cost figures, total-cost-of-ownership models, bounty payouts, insurance figures, or liability caps appear anywhere in the writing.
...
CNML is not described as the OIML equivalent of DCC. The two formats operate at different tiers of the metrology infrastructure and are complementary.

CNML is not associated with NIST or any other external evaluation program. NIST is mentioned only where the writing cites a real NIST standard such as FIPS 203, FIPS 204, or NIST SP 800-53.

Local filesystem paths such as `~/src/` or `/Users/` do not appear in any committed file. Cross-repository references describe the upstream project by name.

## When uncertain

If a draft contains a claim that cannot be verified against a published source, the claim is removed. If a draft makes the writer uncertain whether a pattern is permitted, the pattern is treated as prohibited. Vagueness is acceptable. Invented specifics are not.
